...
class MainActivity : AppCompatActivity() {
    ...
    private fun initViewModel() {
        with(viewModel) {
            subscribe(uiLabels, ::handleUiRouter)
            launchAndRepeatWithViewLifecycle { viewModel.uiRouter.collect(::handleState) }
        }
    }
    ...
    private fun handleUiRouter(router: MainView.Router): Unit = when (router) {
        MainView.Router.Back -> Unit
        is MainView.Router.NavigateTo -> moveToFragment(router.fragment)
