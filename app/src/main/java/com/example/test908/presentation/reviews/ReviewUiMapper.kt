...
class ReviewUiMapper @Inject constructor(@ApplicationContext val context: Context) {
    fun convertTimer(time: Long): String {
        ...
        return if (min.toInt() == 0 && hour.toInt() == 0) {
            ...
            "${context.resources.getQuantityString(R.plurals.timer_days,day.toInt(),day)} ${context.resources.getQuantityString(
                R.plurals.timer_hours,
                hour.toInt(),
                hour
            )}"
        } else {
            "${context.resources.getQuantityString(R.plurals.timer_minutes,min.toInt(),min)} ${context.resources.getQuantityString(
                R.plurals.timer_sec,
                sec.toInt(),
                sec
            )}"
